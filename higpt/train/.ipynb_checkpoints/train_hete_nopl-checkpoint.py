...
from hetegpt.train.graphchat_trainer import GraphChatTrainer

from hetegpt import conversation as conversation_lib
from hetegpt.model import *
...
class DataArguments:
    ...
    num_shot: Optional[int] = field(default=1)
...
class LazySupervisedDatasetHetero(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        for i, path in enumerate(data_path):
            ann_file = glob.glob(osp.join(graph_root, path, 'ann_processed_MetaHGT_imdb_dblp_epoch5/cot', '*.json'), recursive=True)
        ...
        self.node_feas_dict_dblp = torch.load('./hetegpt/model/meta_hgt/meta_dict/dblp/node_type.pt')
        for k in self.node_feas_dict_dblp.keys():
            self.node_feas_dict_dblp[k] = torch.Tensor(self.node_feas_dict_dblp[k])
        self.edge_feas_dict_dblp = torch.load('./hetegpt/model/meta_hgt/meta_dict/dblp/edge_type.pt')
        for k in self.edge_feas_dict_dblp.keys():
            self.edge_feas_dict_dblp[k] = torch.Tensor(self.edge_feas_dict_dblp[k])

        self.node_feas_dict_acm = torch.load('./hetegpt/model/meta_hgt/meta_dict/acm/node_type.pt')
        for k in self.node_feas_dict_acm.keys():
            self.node_feas_dict_acm[k] = torch.Tensor(self.node_feas_dict_acm[k])
        self.edge_feas_dict_acm = torch.load('./hetegpt/model/meta_hgt/meta_dict/acm/edge_type.pt')
        for k in self.node_feas_dict_acm.keys():
            self.node_feas_dict_acm[k] = torch.Tensor(self.node_feas_dict_acm[k])

        self.node_feas_dict_imdb = torch.load('./hetegpt/model/meta_hgt/meta_dict/imdb/node_type.pt')
        for k in self.node_feas_dict_imdb.keys():
            self.node_feas_dict_imdb[k] = torch.Tensor(self.node_feas_dict_imdb[k])
        self.edge_feas_dict_imdb = torch.load('./hetegpt/model/meta_hgt/meta_dict/imdb/edge_type.pt')
        for k in self.node_feas_dict_imdb.keys():
            self.node_feas_dict_imdb[k] = torch.Tensor(self.node_feas_dict_imdb[k])
