...
cot_case_mapping = {
    ...
    5: "Please generate some knowledge that can assist in formulating an answer, including, but not limited to: distinctions between the four categories. \nImagine that you have arrived at the correct answer based on the provided information and knowledge, and present a step-by-step reasoning.\nUsing the following format:\nKnowledge: ...\nAnswer: ...\nReason: ...",
...
json_ann_dir = wd / 'hi_datasets/stage2_data/DBLP/instruct_ds/ann_processed/DBLP_train_std_0_400.json'
...
def query_gpt(s_idx, e_idx, cot_case):
    ...
    if osp.exists(osp.join(file_dir, 'hi_datasets/stage2_data/DBLP/instruct_ds/ann_processed/cot')) is False: 
        os.makedirs(osp.join(file_dir, 'hi_datasets/stage2_data/DBLP/instruct_ds/ann_processed/cot'))
        instruct_list = []
        data_dir = osp.join(file_dir, 'hi_datasets/stage2_data/DBLP/instruct_ds/ann_processed/cot')
        with open(osp.join(data_dir, f'dblp_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'w', encoding='utf-8') as f:
            json.dump(instruct_list, f, ensure_ascii=False)
    else: 
        data_dir = osp.join(file_dir, 'hi_datasets/stage2_data/DBLP/instruct_ds/ann_processed/cot')
        if osp.exists(osp.join(data_dir, f'dblp_cot_pred_{s_idx}_{e_idx}_{cot_case}.json')) is False: 
            instruct_list = []
            with open(osp.join(data_dir, f'dblp_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'w', encoding='utf-8') as f:
                json.dump(instruct_list, f, ensure_ascii=False)
        else: 
            with open(osp.join(data_dir, f'dblp_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'r', encoding='utf-8') as f:
    ...
    try:
        ...
        del instruct_list[-1]
        with open(osp.join(data_dir, f'dblp_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'w') as f:
