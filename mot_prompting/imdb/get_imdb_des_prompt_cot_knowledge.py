...
cot_case_mapping = {
    ...
    6: "Please generate some knowledge that can assist in formulating an answer, including, but not limited to: background knowledge of the given movie, actors and director. \nImagine that you have arrived at the correct answer based on the provided information and knowledge, and present a step-by-step reasoning.\nUsing the following format:\nKnowledge: ...\nAnswer: ...\nReason: ..."
}
...
json_ann_dir = wd / 'hi_datasets/stage2_data/IMDB/instruct_ds_imdb/ann/IMDB_train_std_0_400.json'
...
def query_gpt(s_idx, e_idx, cot_case):
    ...
    if osp.exists(osp.join(file_dir, 'hi_datasets/stage2_data/IMDB/instruct_ds_imdb/ann/cot')) is False: 
        os.makedirs(osp.join(file_dir, 'hi_datasets/stage2_data/IMDB/instruct_ds_imdb/ann/cot'))
        instruct_list = []
        data_dir = osp.join(file_dir, 'hi_datasets/stage2_data/IMDB/instruct_ds_imdb/ann/cot')
        with open(osp.join(data_dir, f'IMDB_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'w', encoding='utf-8') as f:
            json.dump(instruct_list, f, ensure_ascii=False)
    else: 
        data_dir = osp.join(file_dir, 'hi_datasets/stage2_data/IMDB/instruct_ds_imdb/ann/cot')
        if osp.exists(osp.join(data_dir, f'IMDB_cot_pred_{s_idx}_{e_idx}_{cot_case}.json')) is False: 
            instruct_list = []
            with open(osp.join(data_dir, f'IMDB_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'w', encoding='utf-8') as f:
                json.dump(instruct_list, f, ensure_ascii=False)
        else: 
            with open(osp.join(data_dir, f'IMDB_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'r', encoding='utf-8') as f:
    ...
    try:
        ...
        del instruct_list[-1]
        with open(osp.join(data_dir, f'IMDB_cot_pred_{s_idx}_{e_idx}_{cot_case}.json'), 'w') as f:
...
if __name__ ==  "__main__":
    ...
    parser.add_argument('--cot_case', default=0, type=int)
